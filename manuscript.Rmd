...
<!--M2A: I think you mentioned you might have someone do this check, but let me know if I misunderstood!-->
<!-- a2m we then decided we'd both feel more certain if you did... -->


<!-- M2A: regarding demo, it looks like you have more mateds than birth orders because family data was systematically not collected for Pu. kids and was missing for three kids tested in No. who were actually from other hamlets. New xslx files sent via email! I left age recomputing to you since I was confused about which file would be used.-->
<!-- a2m thanks!!! now inc'ed -->

<!-- M2A: regarding NAs in stim_seg_freq, these appear for 'lv' stimuli because, in the 'segments' table, freq_corpus and freq_corpus_types are NA for that grapheme). The 'segments' table has NAs for 'lv' because it is not included in segment-counts.txt or segment-counts-types.txt. Note that that these NAs are excluded when adding mean item frequency to the 'stims' table (wrangling.R/l.136 & l.145), but this has a pretty big impact since those are all two-segment words. Not sure how avoidable this is, going back to our discussion before about not wanting to ad arbitrarily small frequencies. Thoughts? -->
<!-- a2m So your fear is that frequency is overestimated in these cases, right? Perhaps we can assign the missing segments the lowest observed frequencies? -->
...
<!-- M2A: Makes sense. I was thinking we could already hint at YD as interesting in particular bc of the phonology and not simply because it's a random one of many previously untested populations. I don't mind adding something like this in once I'm done with the demo check.-->
<!-- a2m added ", and mostly with languages having phonological inventories that are small to moderate in size" + notice the " with an unusually dense phonological inventory"; my feeling is that it may interrupt the flow to add a lot more here... don't you think? -->
...
read.csv("nwr-demo.csv",header=T,sep=",")->demo
#colnames(demo)
...
[^2]: We also carried out this analysis using token phone frequency, but this measure was not correlated with whole-item NWR scores, and therefore the fact that it did not explain away the predictive value of cross-linguistic phone frequency was less convincing.
...
Setting monosyllables aside, we observe the typical pattern of lower scores for longer items, although this is particularly salient for the whole word scoring. This is the most commonly reported type of score, but it is also the least forgiving. The pattern is less marked when other two scores are used, which are less sensitive to errors. Averaging across participants and items, the Pearson correlation between length (2-4 syllables) and whole-item NWR scores was r(`r length_r$parameter`) = `r round(length_r$estimate,3)`.
<!-- a2m it doesn't make sense to do this, right, with only 3 items? -->
<!-- M2A: you mean with only three lengths? I guess I would prefer a little regression -->
