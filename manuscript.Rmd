...
aggregate(trnsc$correct*100,by=list(trnsc$id,trnsc$Sex,trnsc$age.exact,trnsc$birthOrder,trnsc$matEd),mean)->overall
aggregate(trnsc$pc*100,by=list(trnsc$id,trnsc$Sex,trnsc$age.exact,trnsc$birthOrder,trnsc$matEd),mean)->pc
aggregate(trnsc$nld*100,by=list(trnsc$id,trnsc$Sex,trnsc$age.exact,trnsc$birthOrder,trnsc$matEd),mean)->nld
...
We investigated patterns of deletion and substitution. Deletions were relatively rare, with only `r sum(del_tab[c(oralVs,nasalVs)])` vowels deleted, and `r sum(del_tab[c(simplexCs,complexCs)])` consonants. 
...
```{r bylength}
aggregate(trnsc$correct*100,by=list(trnsc$id,trnsc$age.exact,trnsc$type),mean)->by_length
aggregate(trnsc$pc*100,by=list(trnsc$id,trnsc$age.exact,trnsc$type),mean)->pc
aggregate(trnsc$nld*100,by=list(trnsc$id,trnsc$age.exact,trnsc$type),mean)->nld
