...
trnsc$target.ortho=gsub("([aeiouêâéáóî])\\1+", "\\1", trnsc$target.ortho)

# if correct=0 but orthotarget=mispronunciation (probably due to length errors) then change correct to 1 
#sum(trnsc$correct==0 & trnsc$target.ortho == trnsc$mp & !is.na(trnsc$mp)) #this only happens twice
trnsc$correct[trnsc$correct==0 & trnsc$target.ortho == trnsc$mp & !is.na(trnsc$mp)]<-1
...
for(i in grep("0",trnsc$correct)){
 #lev
  ...
  trnsc$tarlen[i]=nchar(trnsc$target.ortho[i])
...
# for(i in dim(correspondances)[1]:1) { #transform into orthography
#   substitution_bank=gsub(correspondances[i,2],correspondances[i,1],substitution_bank)
#   deletion_bank=gsub(correspondances[i,2],correspondances[i,1],deletion_bank)
# }
