...
```{r}
read.csv("prevlit.csv")->kids

```


@ac ADD TABLE SUMMARIZING PREVIOUS WORK
...
The child was donned a headset (**xx @mc please fill in** for most of the children, xx @ac fill in for the rest). For most children, the headset could not stay comfortably on the child's head, and thus it was placed on the child's shoulders, with the microphone carefully placed close to the child's mouth. A local informant sat next to the child, to  provide the instructions and, if needed, coach the child to make sure that they understood that the task was to repeat the items precisely without changing anything using the three practice items as well as real words. The experimenter (the first author) delivered the elicitation stimuli to the local informant and the child over headhphones. 
...
# merge(ms,onl,all=T)->ms
# merge(ms,demo,all=T)->scores
...
read.csv("NWR-transcription.csv",sep=";")->coding

#remove some weird items
coding[coding$item!="task4",]->coding

read.csv("final_order_ful.txt",sep="\t")->coding_key
coding_key$row=row.names(coding_key)

merge(coding,coding_key,all=T)->joint

merge(joint,demo,by.x="id",by.y="ID",all.x=T)->joint

#order so we have all the items by a given child in the right chrono order
joint[order(joint$file, joint$int),]->joint

#add variable coding whether it's the child's first or subsequent attempts
joint$previous<-c(NA,as.character(joint$target[1:(dim(joint)[1]-1)]))
joint$attempt<-ifelse(joint$target!=joint$previous,"first","subsequent")
joint$attempt[1]<-"first"

#remove practice items
practice=joint[(joint$target %in% c("wi","poni","nopimade")),]
joint=joint[!(joint$target %in% c("wi","poni","nopimade")),]

read.csv("stimuli.csv",header=T)->stims

# phonologize things
merge(joint,stims,by="target",all=T)->joint
joint$target_pron=as.character(joint$ortho)

joint$pron=as.character(joint$mispronunciation)

correspondances=matrix( #list correspondances always by pairs
  c(
...
for(i in 1:dim(correspondances)[1]) {
  joint$pron=gsub(correspondances[i,1],correspondances[i,2],joint$pron)
  joint$target_pron=gsub(correspondances[i,1],correspondances[i,2],joint$target_pron)
}
...
aggregate(joint$correct*100,by=list(joint$id,joint$age.exact,joint$attempt),mean)->pwcm_att
...
joint=joint[joint$attempt=="first",]
```


### Overall repetition accuracy

We start by describing whole word accuracy. Participants scored 
