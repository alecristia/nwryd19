...
```{r Fig1-first_vs_others, fig.pos = "!t", fig.width=5, fig.height=5, fig.cap="Whole-item NWR scores for individual participants averaging separately their first attempts and all other attempts."}
...
plot(pwcm_att$Word~myx,pch=20,cex=2,lty=3,xlab="",xaxt="n",ylab="NWR scores (% words)")
...
aggregate(trnsc$correct*100,by=list(trnsc$avg_fr_s),mean)->xl_fr_tab
...
aggregate(trnsc$correct*100,by=list(trnsc$avg_fr_cor_ty_s),mean)->cor_fr_tab
...
aggregate(trnsc$correct*100,by=list(trnsc$type),mean)->by_length_no_child
...
len_lm=glmer(correct~type+age.rounded_s+(1|file) + (1|target),data=trnsc,family="binomial",subset=c(trnsc$type!=1))
