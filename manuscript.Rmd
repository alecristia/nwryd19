...
NWR has been used to seek answers to a variety of theoretical questions, including what the links  between phonology, working memory, and the lexicon are [@bowey2001nonword], and how extensively phonological constraints found in the lexicon affect online production [@gallagher2014identity]. NWR is also frequently used in applied contexts, notably as a diagnostic tool for language delays and disorders [@chiat2015non;@estes2007differences]. Since non-words can be generated in any language, it has attracted the attention of researchers working in multilingual and linguistically diverse environments, particularly in Europe [@meir2016disentangling;@is08042009language;@chiat2015nonword;@armon2015assessing].  Non-words can be designed to try to isolate more narrowly certain skills; for instance, one can choose non-words that contain real morphemes in order to load more on prior language experience, or non-words that are shorter to avoid loading on working memory [see a discussion in @chiat2015nonword]. Broadly, however, NWR scores will necessarily reflect to a certain extent long-term phonological knowledge (to perceive the item precisely despite not having heard it before) as well as online phonological working memory (to encode the item in the interval between hearing it and saying it back) and flexible production patterns (to produce the item precisely despite not having pronounced it before).  
...
It is possible that differences are due to language characteristics, including the modal length of words in the language and/or in child-directed speech in that culture -- a hypothesis discussed for instance in @chiat2015non (pp. 7-8; see also p. 5). In broad terms, one may expect languages with a lexicon that is heavily biased towards monosyllables to show greater length effects than languages where words are modally longer. A non-systematic meta-analyses does not provide overwhelming support for this hypothesis [@cristia2021supplementary; SM1].  

Nonetheless, given the paucity of research looking at this question, and the diversity of current results, we do not approach this issue within a hypothesis-testing framework but sought instead to provide one more piece of data on the question, which may be re-used in future meta- or mega-analytic analyses.
...
We then added a mention in the discussion that our work does not allow us to tease apart lexical knowledge from phonological knowledge and from working memory in ways that have been done in previous research.
