...
Taking into account only the first attempts, the overall repetition accuracy measured in whole words is `r round(mean(firstp$cor.online)*100)`% based on the online coding (i.e., whether the native experimenter asked for the target to be re-played), and `r round(mean(firstp$correct)*100)`% when the offline coding is considered. On average, the phoneme-based normalized Levenshtein distance was `r round(mean(firstp$nld)*100)`%, meaning that about 20% of phonemes were substituted, inserted, or deleted.
...
```{r figlength,eval=F}
#not working
aggregate(firstp$correct,list(firstp$tarlen,firstp$id),mean)
plot(firstp$correct~firstp$tarlen)
